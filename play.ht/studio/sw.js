...
const Y = s => new URL(String(s), location.href).href.replace(new RegExp(`^${location.origin}`), "");
try {
...
class B {
    constructor(e) {
        this.cacheWillUpdate = async ({
            response: t
        }) => this._cacheableResponse.isResponseCacheable(t) ? t : null, this._cacheableResponse = new Z(e)
    }
}
const F = new Set;
...
async function ce(s, e) {
    let t = null;
    if (s.url && (t = new URL(s.url).origin), t !== self.location.origin) throw new l("cross-origin-copy-response", {
        origin: t
    });
    const n = s.clone(),
        a = {
...
let N = {
    get(s, e, t) {
        if (s instanceof IDBTransaction) {
            if (e === "done") return P.get(s);
            if (e === "objectStoreNames") return s.objectStoreNames || V.get(s);
...
function me(s) {
    return s === IDBDatabase.prototype.transaction && !("objectStoreNames" in IDBTransaction.prototype) ? function(e, ...t) {
        ...
        return s.apply(T(this), e), f(q.get(this))
    } : function(...e) {
...
function we(s) {
    return typeof s == "function" ? me(s) : (s instanceof IDBTransaction && pe(s), le(s, ue()) ? new Proxy(s, N) : s)
}

function f(s) {
    if (s instanceof IDBRequest) return fe(s);
    if (k.has(s)) return k.get(s);
    const e = we(s);
    return e !== s && (k.set(s, e), I.set(e, s)), e
}
const T = s => I.get(s);
...
function ye(s, e, {
    ...
    })), t && i.addEventListener("blocked", (o => t(o.oldVersion, o.newVersion, o))), c.then((o => {
...
function _e(s, {
    ...
    return e && t.addEventListener("blocked", (n => e(n.oldVersion, n))), f(t).then((() => {}))
}
...
function W(s, e) {
    ...
    if (L.get(e)) return L.get(e);
    const t = e.replace(/FromIndex$/, ""),
        n = e !== t,
        a = be.includes(t);
    if (!(t in (n ? IDBIndex : IDBObjectStore).prototype) || !(a || Re.includes(t))) return;
    const r = async function(i, ...c) {
        const o = this.transaction(i, a ? "readwrite" : "readonly");
        let h = o.store;
        return n && (h = h.index(c.shift())), (await Promise.all([h[t](...c), a && o.done]))[0]
    };
    return L.set(e, r), r
}
ge((s => ({ ...s,
...
class $ {
    constructor(e = {}) {
        ...
        }, this._config = e, this._maxAgeSeconds = e.maxAgeSeconds, this._cacheExpirations = new Map, e.purgeOnQuotaError && ee((() => this.deleteCacheAndMetadata()))
    }
...
function ke(s) {
    if (!s) throw new l("add-to-cache-list-unexpected-type", {
        entry: s
    });
    if (typeof s == "string") {
...
class Ue {
    constructor(e, t) {
        this._cacheKeys = {}, Object.assign(this, t), this.event = t.event, this._strategy = e, this._handlerDeferred = new ae, this._extendLifetimePromises = [], this._plugins = [...e.plugins], this._pluginStateMap = new Map;
    ...
    async fetch(e) {
        ...
        try {
            for (const i of this.iterateCallbacks("requestWillFetch")) n = await i({
                request: n.clone(),
                event: t
            })
        } catch (i) {
    ...
    async cacheMatch(e) {
        ...
        for (const o of this.iterateCallbacks("cachedResponseWillBeUsed")) n = await o({
            cacheName: a,
            matchOptions: r,
            cachedResponse: n,
            request: i,
    ...
    async cachePut(e, t) {
        ...
        const {
            cacheName: i,
            matchOptions: c
        } = this._strategy, o = await self.caches.open(i), h = this.hasCallback("cacheDidUpdate"), g = h ? await se(o, a.clone(), ["__WB_REVISION__"], c) : null;
        try {
            await o.put(a, h ? r.clone() : r)
        } catch (u) {
            if (u instanceof Error) throw u.name === "QuotaExceededError" && await re(), u
        }
    ...
    async getCacheKey(e, t) {
        ...
        if (!this._cacheKeys[n]) {
            let a = e;
            for (const r of this.iterateCallbacks("cacheKeyWillBeUsed")) a = b(await r({
                mode: t,
                request: a,
                event: this.event,
                params: this.params
...
class p extends M {
    ...
    async _handleInstall(e, t) {
        ...
        if (!await t.cachePut(e, n.clone())) throw new l("bad-precaching-response", {
            url: e.url,
            status: n.status
        });
        return n
    ...
    _useDefaultCacheabilityPluginIfNeeded() {
        let e = null,
            t = 0;
        for (const [n, a] of this.plugins.entries()) a !== p.copyRedirectedCacheableResponsesPlugin && (a === p.defaultPrecacheCacheabilityPlugin && (e = n), a.cacheWillUpdate && t++);
...
class Pe {
    constructor({
        ...
        this._urlsToCacheKeys = new Map, this._urlsToCacheModes = new Map, this._cacheKeysToIntegrities = new Map, this._strategy = new p({
    ...
    addToCacheList(e) {
        ...
        for (const n of e) {
            ...
            const {
                cacheKey: a,
                url: r
            } = ke(n), i = typeof n != "string" && n.revision ? "reload" : "default";
            ...
            if (typeof n != "string" && n.integrity) {
                if (this._cacheKeysToIntegrities.has(a) && this._cacheKeysToIntegrities.get(a) !== n.integrity) throw new l("add-to-cache-list-conflicting-integrities", {
                    url: r
                });
    ...
    addCacheListener() {
        self.addEventListener("message", (e => {
            if (e.data && e.data.type === "CACHE_URLS") {
                const {
                    payload: t
                } = e.data, n = Promise.all(t.urlsToCache.map((a => {
                    typeof a == "string" && (a = [a]);
...
function E(s, e, t) {
    ...
    if (typeof s == "string") {
        ...
        n = new R(i, e, t)
    } else if (s instanceof RegExp) n = new Ne(s, e, t);
...
function Ke(s, e = []) {
    for (const t of [...s.searchParams.keys()]) e.some((n => n.test(t))) && s.searchParams.delete(t);
    return s
}
...
const Fe = s => s.origin === location.origin;
We([{
    revision: null,
    url: "assets/index-4ecad42e.js"
}, {
    revision: null,
    url: "assets/index-ffd2f56d.css"
}, {
    revision: "573acdb7a8f3250891d80d0b5a0c6a78",
    url: "registerSW.js"
}, {
    revision: "4c15f79583a9c95dd1a04af5d900763d",
    url: "favicon.ico"
}, {
    revision: "499fa714f87e64c940864f2e8469ecea",
    url: "apple-touch-icon.png"
}, {
    revision: "300a1eda188bcf5d546007fe75b8df28",
    url: "pwa-192x192.png"
}, {
    revision: "4a8e5aad1deb122f46aa75e5fcac051c",
    url: "pwa-640x640.png"
}, {
    revision: "ef439fe82410d798ef78ddc08e20bbea",
    url: "pwa-maskable.svg"
}, {
    revision: "104ba51e3ca40143f290f87028dd962b",
    url: "manifest.webmanifest"
}]);
E((({
    request: s
}) => s.destination === "document"), new z({
...
E((({
    request: s,
    url: e
}) => Fe(e) && (s.destination === "script" || s.destination === "style" || s.destination === "image")), new je({
...
E((({
    url: s
}) => !s.pathname.match(/^[^.]+$|\\.(?!(css|gif|ico|jpg|js|png|txt|svg|woff|ttf|map|json|webmanifest)$)([^.]+$)/)), new z({
    cacheName: "html-cache"
}));
he();
oe();